...
def fetch(url):
  sublist = map(str.lower,open(os.path.dirname(__file__)+'/showlist','r').read().split('\n'))
  ...
  tr_item = re.compile('<tr class="(?:even|odd)" id="torrent_uploaded_torrents\d+">.*?<a title="Download torrent file" href="(.*?)" class="idownload icon16">.*?<a href=".*?" class="cellMainLink">(.*?) (?:S(\d+)E(\d+)|(\d+) (\d+ \d+)).*?\[ettv\]</a>.*?<td class="center">(\d+)&nbsp;hours</td>.*?<td class="green center">(\d+)</td>.*?</tr>',re.S) 

  # fetch uploads page
  # this page is sorted by age, so we don't need to concern ourselves with it
  ...
  # for each unique key, crossmatch it with subscription list and pick the most seeded torrent.
  # a "None" value is added to each match as a failsafe if there is no hit. They are removed at the end.
...
if __name__ == '__main__':
    #subscription source
    subs_url  = 'http://kickass.to/user/ettv/uploads/'
    #download directory
    directory = os.path.dirname(__file__)+'/torrents'
